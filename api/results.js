...
async function fetchEciResults(){
  ...
  if(constituencies.length){
    return {source:'eci-live', message:'ECI live parsed from results.eci.gov.in', updatedAt:new Date().toISOString(), constituencies};
  }
  const notStarted = /Results trends will start/i.test(home);
  return {source:'eci-checked', message:notStarted?'Trends begin at 8:00 AM on 4 May 2026.':'ECI page found but result tables were not parseable yet.', updatedAt:new Date().toISOString(), constituencies:[]};
}
...
export default async function handler(req, res){
  ...
  try{
    const payload = await fetchEciResults();
    res.status(200).json(payload);
  }catch(error){
    res.status(200).json({source:'offline-fallback', message:'ECI fetch failed. Using bundled fallback data.', updatedAt:new Date().toISOString(), error:error.message, constituencies:[]});
  }
}
